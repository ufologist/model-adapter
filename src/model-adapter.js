...
export default class ModelAdapter {
     ...
     * @param {*} [source] 源数据模型
     * @param {boolean} [copy = true] 是否自动一对一映射源数据上的属性. 默认自动 copy, 因为前端数据模型一般用于适配后端接口返回的数据模型, 后端接口字段变更的情况是较少的, 因此如果全部字段都需要适配一遍就很累赘, 因为大部分字段我们是一对一映射的, 再加上后端字段变更的情况又较少, 这样做显然收益不大, 反而增加了使用的门槛. 但如果需要做到前后端数据模型彻底解耦, 建议关闭 copy 功能, 一个一个属性的适配, 明确声明前端模型有哪些属性.
     */
